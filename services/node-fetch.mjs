...
const getAnime = async (startId, endId) => {
	...
	for (let i = startId; i <= endId; i++) {
		try {
			...
			if (response.status === 200) {
				...
				const partialHotEncodedAnime = hotEncodeAnime(
					anime,
					uniqueGenres,
					ageRatingMapping
				);
				writeFileSync(
					...
					JSON.stringify(partialHotEncodedAnime, null, 2)
				);
			}
	...
	const animeWithHotEncodedGenres = hotEncodeAnime(
		anime,
		uniqueGenres,
		ageRatingMapping
	);
